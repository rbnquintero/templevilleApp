...
class HeaderIOS extends Component {
  ...
  render() {
    ...
    const titleColor = foreground === 'dark' ? Colors.darkTextN : 'white';
    const itemsColor = foreground === 'dark' ? Colors.lightTextN : 'white';

    const content = React.Children.count(this.props.children) === 0
      ? <TextN style={[styles.titleText, {color: titleColor}]}>
          {title}
        </TextN>
      : this.props.children;
    return (
...
class ItemWrapperIOS extends Component {
  ...
  render() {
    ...
    if (layout !== 'icon' && title) {
      content = (
        <TextN style={[styles.itemTextN, {color}]}>
          {title.toUpperCase()}
        </TextN>
      );
    } else if (icon) {
